/**
 * Read-only routes for MoneyPrinter / social clips (no JWT).
 * Mounted at /mp so production /posts/:id never captures "public-sample".
 * Opt out: DISABLE_PUBLIC_POST_SAMPLE=1
 */
...
// GET /mp/sample — random public image post + handle pool
router.get('/sample', async (_req: Request, res: Response) => {
  ...
  try {
    ...
    const handlePool = poolRows.map((r: { username: string }) => r.username).filter(Boolean);
...
// GET /mp/sample/post/:postId
router.get('/sample/post/:postId', async (req: Request, res: Response) => {
  if (disabled()) {
    res.status(404).json({ error: 'Not found' });
    return;
  }
  ...
  try {
    const { rows: [post] } = await pool.query(
      `SELECT p.id, p.caption, u.username AS author_username,
       ...
       FROM posts p
       JOIN users u ON u.id = p.author_id
       WHERE p.id = $1::uuid AND p.scope = 'everyone' AND p.is_reported = FALSE
         AND EXISTS (SELECT 1 FROM post_images pi2 WHERE pi2.post_id = p.id)`,
      [postId]
    );
    if (!post) {
...
// GET /mp/sample/:postId/comments
router.get('/sample/:postId/comments', async (req: Request, res: Response) => {
  if (disabled()) {
    res.status(404).json({ error: 'Not found' });
    return;
  }
...
// GET /mp/usernames
router.get('/usernames', async (_req: Request, res: Response) => {
  if (disabled()) {
    res.status(404).json({ error: 'Not found' });
    return;
  }
  try {
    ...
    res.json(rows.map((r: { username: string }) => r.username).filter(Boolean));
  } catch (err: any) {
    console.error(err);
    res.status(500).json({ error: 'Server error', details: err.message });
  }
